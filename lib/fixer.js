...
function fixBlob(content, lastCommit, blob, treeBlob, match) {
    var newBlobContent;
...
function* fixMatch(githubApi, content, match) {
    ...
    return fixBlob(content, lastCommit, modifiedFile, blob, match);
}

function* fixTypo(content, githubApi) {
    var index;
    var match;
    var fix;
    var fixes = [];
