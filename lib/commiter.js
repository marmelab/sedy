...
function* digestCommit(githubApi, content, typoFix) {
    var author = {
...
module.exports = function* committer(fixedContent, githubApi) {
    var index;
    var message;
    var commits = [];
    var commitIds = [];
    ...
    for (index = 0; index < fixes.length; index++) {
        commits.push(digestCommit(githubApi, content, fixes[index]));
    }
